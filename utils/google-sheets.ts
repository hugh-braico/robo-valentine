...
export async function initGoogleSheet(): Promise<void> {
    ...
    await doc.loadInfo();
    console.log(`  Successfully accessed Google sheet: ${doc.title}.`);
}
...
async function loadMovesAndAliases(): Promise<void> {
    ...
    for (const characterRow of characterRows) {
        ...
        console.log(`      Creating move and alias data for character ${characterName}...`)
        for (const moveRow of moveRows) {
            ...
            for (const pattern of regexAliasList) {
                ...
                // console.log(`        Storing RegexAlias: ${JSON.stringify({Character: characterName, Pattern: pattern, 'Move Name': moveName})}`);
